...
const COVALENT_RADII =  {
  ...
  function visualizeStructure(data, tolerance = 0.3, viewerElement = document.getElementById('viewer')) {
    ...
    // MIC: wrap cartesian coordinates to within unit cell
    function wrapToCell(cart) {
      const frac = math.multiply(invCell, cart);
      const wrappedFrac = frac.map(f => ((f % 1) + 1) % 1);
      const wrappedCart = math.multiply(cell, wrappedFrac);
      return wrappedCart;
    }
  ...
  // function visualizeStructure(data, tolerance = 0.3, viewerElement = document.getElementById('viewer')) {
  //   console.log("👁 visualizeStructure called");
  //   console.log("Viewer element:", viewerElement);
  //   console.log("Structure data:", data);

  //   if (!viewerElement) {
  //     console.error("🚫 viewerElement not found");
  //     return;
  //   }

  //   if (!Array.isArray(data.positions) || !Array.isArray(data.symbols) || data.positions.length !== data.symbols.length) {
  //     viewerElement.innerHTML = `<p class="text-danger text-center">Invalid structure format.</p>`;
  //     console.error("🚫 Invalid structure format", data);
  //     return;
  //   }
  ...
  //     viewerElement.innerHTML = `<p class="text-danger text-center">3Dmol.js is not loaded.</p>`;
  //     return;
  //   }

  //   console.log("✅ Starting viewer creation");
  ...
  //   // const { positions, symbols, cell } = data;
  //   const { positions, symbols } = data;
  ...
  //       center: { x: pos[0], y: pos[1], z: pos[2] },
  //       radius: 0.5,
  //       color: ATOM_COLORS[atom] || ATOM_COLORS['X']
  //     });
  //   }

  //   // Only use cell for distance
  //   for (let i = 0; i < numAtoms; i++) {
  ...
  //       } catch (e) {
  //         console.warn(`⚠️ Bond error between atoms ${i} and ${j}`, e);
  //       }
  ...
  //     console.error("addVec received invalid arguments:", v1, v2);
  //     return [0, 0, 0];
  //   }
  ...
  //   const invCell = math.inv(cell);
  //   const frac = math.multiply(invCell, d);
  //   const wrapped = frac.map(x => x - Math.round(x));
  //   const cartWrapped = math.multiply(cell, wrapped);

  //   return Math.sqrt(cartWrapped[0] ** 2 + cartWrapped[1] ** 2 + cartWrapped[2] ** 2);
  // }
